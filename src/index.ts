...
(async () => {
  ...
  const client = new TelegramClient(storeSession, Config.TELEGRAM_APP_ID, Config.TELEGRAM_APP_HASH, {
    connectionRetries: 5,
  });
  ...
  await client.start({
    ...
    phoneCode: async (tr) => await input.text("Enter your Telegram Verification Code ?"),
